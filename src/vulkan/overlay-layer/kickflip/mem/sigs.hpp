...
class sigs{
    public:
        const char *trace_sig = "E8 ? ? ? ? 48 23 9D ? ? ? ?";
        ...
        const char *C4_sig = "48 89 87 ? ? ? ? 48 8D 3D ? ? ? ? E8 ? ? ? ? 4C 89 E7 4C 8B 65 F8";
       ...
       uintptr_t getViewMatrixOffset()
        ...
        uintptr_t getPlantedC4Offset()
        {
            ...
            /*
            start* 
           .text:0000000000E89540 48 89 87 88 09 00 00                    mov     [rdi+988h], rax
            .text:0000000000E89547 48 8D 3D 52 22 8A 02                    lea     rdi, CPlantedC4

            48 89 87 ?? ?? ?? ?? 48 8D 3D ?? ?? ?? ?? E8 ?? ?? ?? ?? 4C 89 E7 4C 8B 65 F8
            */
           ...
           //if (memcmp(bytes_at_offset, signature, sizeof(signature)) == 0) { //THE RIGHT WAY
            ...
            uint32_t final = 0;
             unsigned char* bytes_at_offset = (unsigned char*)(offset);
            if(bytes_at_offset[0] == 0x48 && bytes_at_offset[1] == 0x89 && bytes_at_offset[2] == 0x87) { //48 89 87 ?? ?? ?? ??
                bytes_at_offset = (unsigned char*)(offset + 0x7);
